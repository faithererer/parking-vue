    ...
    <div class="scan">
      ...
      <el-card style="width: 35%">
        <div class="scanFormBox">
          <h2 style="padding-bottom: 40px;">出场车辆扫描结果</h2>
          <el-form
            :model="scanForm"
            :rules="rules"
            ref="scanFormRef"
          ...
          >
            ...
            <el-form-item label="出场时间" prop="createTime">
            ...
            <el-form-item label="选择停车场" prop="parkId">
              <el-select v-model="scanForm.parkId" placeholder="请选择停车场">
                <el-option
            ...
            <el-form-item>
              <div class="formButton">
                <el-button type="primary" size="small" round @click="exitCar"
                  >确认出场</el-button
                >
...
import exitCarApi from "@/api/scanAndOrder";
import { mapGetters } from "vuex";
import parkApi from "@/api/parkManage";
...
export default {
  ...
  methods: {
    ...
    exitCar(){
      this.$refs.scanFormRef.validate((valid) =>{
        if (valid) {
          this.scanForm.exitTime =  this.scanForm.createTime
          exitCarApi.exitCar(this.scanForm).then((res) => {
            //成功提示
            this.$message({
              message: res.message + "停车时间为：" + res.data.parkingDuration +"小时，"+"停车费为：" + res.data.parkFee + "元",
              type: "success",
            });
          })
        }else {
    ...
    handleScanSuccess(res, file) {
      ...
      console.log(res);
      console.log(this.scanForm.exitTime);
    },
  ...
  created() {
    ...
    parkApi.getAllFreeParkInfoList().then((res) => {
