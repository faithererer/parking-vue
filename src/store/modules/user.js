import { login, loginByGithub, logout, getInfo } from '@/api/user'
...
const actions = {
  ...
  oauthLogin({ commit }, userInfo) {
    const { userName, githubId } = userInfo
    return new Promise((resolve, reject) => {
      loginByGithub({ userName: userName.trim(), githubId: githubId }).then(response => {
        const { data } = response
        commit('SET_TOKEN', data.token)
        setToken(data.token)
        resolve()
